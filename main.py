...
def printStatus(sysinput, sysresponse):
    ...
    print(
        f"Respuesta impulsiva:           {impulse_response.raw_expression}\nSeñal introducida al sistema:  {system_input.raw_expression}"
    )
    print(
        f"la grafica ha sido exportada en {cfg.EXPORT_PATH}Respuesta del sistema.pdf"
    )
...
def updateResponse(system_input, impulse_response):
    ...
    system_response = [
        system_input.X_Axis,
        np.convolve(system_input.Y_Axis, impulse_response.Y_Axis, mode="same")
        / cfg.XRes,
    ]
...
def updateGraphs(system_input, system_response):
    plotter(
        [impulse_response.X_Axis, impulse_response.Y_Axis], *cfg.respuesta_impulsiva
    )
...
if __name__ == "__main__":
    # initial function gathering and processing
    clear()
